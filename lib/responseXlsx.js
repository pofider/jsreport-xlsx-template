...
const preview = async (request, response, options) => {
  // I was not able to convert buffer to the stream that would work with multipart parser on the jsreport.net/temp
  // so I need to save it to file which works
  const { filename } = await options.writeTempFile((uuid) => `${uuid}.xlsx`, response.content)
  const { stream } = await options.readTempFileStream(filename)
